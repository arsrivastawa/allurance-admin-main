...
import { INE_ORDERS_ENDPOINT, OFFLINE_SALES_ENDPOINT, ine_offline_sales_ModuleID } from '../../../utils/apiEndPoints';
import ProductTableFiltersResult from '../customer_bills-table-filters-result';
import {
  // RenderCellStock,
  // RenderCellPrice,
  RenderCellPublish,
...
export default function ProductListView(permissions) {
  ...
  const handleViewRow = useCallback(
    (id) => {
      router.push(paths.dashboard.customerbills.details(id));
    },
    [router]
  );
  ...
  const columns = [
    ...
    },
    {
      ...
      flex: 1,
      hideable: false,
      renderCell: (params) => <RenderCellUserPhone params={params} />,
    },
    {
      field: 'created_at',
      ...
      >
        <CustomBreadcrumbs
          ...
          links={[
            ...
            {
              name: 'Customer Bills',
              href: paths.dashboard.customerbills.root,
            },
            { name: 'List' },
          ]}
          action=
          {add_access === 1 && (
            <Button
              ...
              href={paths.dashboard.customerbills.new}
              variant="contained"
