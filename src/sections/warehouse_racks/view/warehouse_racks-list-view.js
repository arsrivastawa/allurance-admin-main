...
import { CATEGORY_ENDPOINT, ine_category_ModuleID, WAREHOUE_RACKS_ENDPOINT, WAREHOUSE_ADD_NAME } from '../../../utils/apiEndPoints';
import ProductTableFiltersResult from '../warehouse_racks-table-filters-result';
import {
  // RenderCellStock,
  // RenderCellPrice,
  RenderCellPublish,
...
} from '../warehouse_racks-table-row';
...
export default function ProductListView(permissions) {
  ...
  const getListingData = async () => {
    try {
      ...
      const response = await ManageAPIsData(WAREHOUE_RACKS_ENDPOINT, 'GET');
  ...
  const handleDeleteRow = useCallback(
    async (id) => {
      try {
        ...
        const apiUrl = `${WAREHOUE_RACKS_ENDPOINT}?id=${id}`;
  ...
  const handleDeleteRows = useCallback(async () => {
    ...
    try {
      ...
      const apiUrl = `${WAREHOUE_RACKS_ENDPOINT}?ids=${deletedIds}`;
      const response = await ManageAPIsData(apiUrl, 'DELETE');
  ...
  const handleEditRow = useCallback(
    (id) => {
      router.push(paths.dashboard.warehouse_racks.edit(id));
    },
    [router]
  );
        ...
        <CustomBreadcrumbs
          ...
          {add_access === 1 && (
            <Button
              ...
              href={paths.dashboard.warehouse_racks.new}
              variant="contained"
