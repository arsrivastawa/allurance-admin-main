...
import { BLOG_ENDPOINT, ine_category_ModuleID } from '../../../utils/apiEndPoints';
import ProductTableFiltersResult from '../blogs-table-filters-result';
import {
  // RenderCellStock,
  // RenderCellPrice,
  RenderCellPublish,
...
export default function ProductListView(permissions) {
  ...
  const getListingData = async () => {
    try {
      ...
      const response = await ManageAPIsData(BLOG_ENDPOINT, 'GET');
  ...
  const handleDeleteRow = useCallback(
    async (id) => {
      try {
        ...
        const apiUrl = `${BLOG_ENDPOINT}?id=${id}`;
  ...
  const handleDeleteRows = useCallback(async () => {
    ...
    try {
      ...
      const apiUrl = `${BLOG_ENDPOINT}?ids=${deletedIds}`;
      const response = await ManageAPIsData(apiUrl, 'DELETE');
  ...
  const handleEditRow = useCallback(
    (id) => {
      router.push(paths.dashboard.blogs.edit(id));
    },
    [router]
  );
      ...
      >
        <CustomBreadcrumbs
          ...
          links={[
            ...
            {
              name: 'Blogs',
              href: paths.dashboard.blogs.root,
            },
            { name: 'List' },
          ]}
          action=
          {add_access === 1 && (
            <Button
              ...
              href={paths.dashboard.blogs.new}
              variant="contained"
