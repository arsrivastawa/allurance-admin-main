...
import { CATEGORY_ENDPOINT, ine_category_ModuleID, MANAGE_TICKET_SUBJECT } from '../../../utils/apiEndPoints';
import ProductTableFiltersResult from '../manage_ticket_subject-table-filters-result';
import {
  // RenderCellStock,
  // RenderCellPrice,
  RenderCellPublish,
...
} from '../manage_ticket_subject-table-row';
...
export default function ProductListView(permissions) {
  ...
  const handleDeleteRow = useCallback(
    async (id) => {
      try {
        ...
        const apiUrl = `${MANAGE_TICKET_SUBJECT}?id=${id}`;
  ...
  const handleDeleteRows = useCallback(async () => {
    ...
    try {
      ...
      const apiUrl = `${MANAGE_TICKET_SUBJECT}?ids=${deletedIds}`;
      const response = await ManageAPIsData(apiUrl, 'DELETE');
  ...
  const handleEditRow = useCallback(
    (id) => {
      router.push(paths.dashboard.manage_ticket_subject.edit(id));
    },
    [router]
  );
        ...
        <CustomBreadcrumbs
          ...
          {add_access === 1 && (
            <Button
              ...
              href={paths.dashboard.manage_ticket_subject.new}
              variant="contained"
