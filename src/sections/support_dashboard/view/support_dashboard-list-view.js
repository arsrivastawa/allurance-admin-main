...
import ProductTableFiltersResult from '../support_dashboard-table-filters-result';
import {
  RenderCellShape,
...
} from '../support_dashboard-table-row';
...
export default function ProductListView(permissions) {
  ...
  const getListingData = async () => {
    try {
      ...
      const response = await ManageAPIsDataWithHeader(SUPPORT_DASHBOARD_RECORDS, fetchMethod);
  ...
  const getCardData = async () => {
    try {
      ...
      const response = await ManageAPIsDataWithHeader(SUPPORT_DASHBOARD_CARD_DATA, fetchMethod);
  ...
  return (
    <>
      ...
      >
        <Typography variant="h3">Support Dashboard</Typography>
