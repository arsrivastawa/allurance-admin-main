...
import {
  AFFILIATE_ENDPOINT,
  CATEGORY_ENDPOINT,
  ine_category_ModuleID,
} from '../../../utils/apiEndPoints';
import ProductTableFiltersResult from '../manage_affiliate-table-filters-result';
import {
  // RenderCellStock,
  // RenderCellPrice,
  RenderCellPublish,
  ...
  RenderCellStock,
} from '../manage_affiliate-table-row';
...
export default function ProductListView(permissions) {
  ...
  const getListingData = async () => {
    try {
      const response = await ManageAPIsData(AFFILIATE_ENDPOINT, 'GET');
  ...
  const handleDeleteRows = useCallback(async () => {
    ...
    try {
      const apiUrl = `${AFFILIATE_ENDPOINT}?ids=${deletedIds}`;
      const response = await ManageAPIsData(apiUrl, 'DELETE');
  ...
  const handleEditRow = useCallback(
    (id) => {
      router.push(paths.dashboard.manage_affiliate.edit(id));
    },
    [router]
  );
  ...
  const handleViewRow = useCallback(
    (id) => {
      router.push(paths.dashboard.manage_affiliate.details(id));
    },
    [router]
  );
          ...
          action={
            add_access === 1 && (
              <Button
                ...
                href={paths.dashboard.manage_affiliate.new}
                variant="contained"
  ...
  if (name) {
    inputData = inputData.filter(
      (invoice) =>
        ...
        invoice.record_status.toLowerCase().indexOf(name.toLowerCase()) !== -1
    );
  }
