...
const TABLE_HEAD = [
  ...
  { id: 'till_date', label: 'End date' },
  { id: 'discount_percentage', label: 'Discount ( in % )' },

  { id: 'min_cart_value', label: 'Minimum cart value' },
  { id: 'min_cart_products', label: 'Minimum cart products' },
...
export default function InvoiceListView(permissions) {
  ...
  const getInvoiceLength = (status) => {
    ...
    //   return 0; // Return 0 if tableData is not a valid array
    // }

    if (status === 'active') {
      // Count the number of records where today's date is between start_date and end_date
      return tableData.filter((item) => {
        const currentDate = new Date();
        return (
          item.record_status === 2 && // Filter for approved invoices
          (new Date() >= new Date(item.start_date) && new Date() <= new Date(item.till_date)) &&
          !(currentDate > new Date(item.till_date)) // Exclude expired invoices
        );
      }).length;
    }
    else if (status === 2) {
      ...
      return tableData.filter((item) => {
        ...
        return (
          item.record_status === 2 && // Filter for approved invoices
          !(currentDate >= new Date(item.start_date) && currentDate <= new Date(item.till_date)) &&
          (currentDate > new Date(item.till_date)) // Exclude expired invoices
        );
      }).length;
    }
    else {
      // Count the number of records with the specified status
      return tableData.filter((item) => item.record_status === status).length;
    }
  };
  ...
  const TABS = [
    { value: 'all', label: 'All', color: 'default', count: tableData.length },
    { value: 'active', label: 'Active', color: 'info', count: getInvoiceLength('active') },
    { value: 'expired', label: 'Expired', color: 'secondary', count: getInvoiceLength('expired') },
  ...
  const handleDeleteRows = useCallback(async () => {
    ...
    const apiUrl = `${CAMPAIGN_ENDPOINT}?ids=${deletedIds}`;
    const responseData = await fetchDataFromApi(apiUrl, 'DELETE');
...
//         invoice.campaign_name.toLowerCase().indexOf(name.toLowerCase()) !== -1
//     );
//   }
...
//       filterInvoicesByStatus = 'rejected';
//     } else if (status === "active") {
//       // Filter invoices based on today's date being between start date and end date
//       inputData = inputData.filter((invoice) => {
//         const currentDate = new Date();
//         return (
//           (currentDate >= new Date(invoice.start_date) && currentDate <= new Date(invoice.till_date)));
//       });
//     }
//     else if (status === "active")
...
function applyFilter({ inputData, comparator, filters, dateError }) {
  ...
  if (name) {
    inputData = inputData.filter(
      (invoice) =>
        // invoice.min_cart_value.toLowerCase().indexOf(name.toLowerCase()) !== -1 ||
        // invoice.discount_percentage.toLowerCase().indexOf(name.toLowerCase()) !== -1 ||
        invoice.campaign_name.toLowerCase().indexOf(name.toLowerCase()) !== -1
    );
  }
  ...
  if (status !== 'all') {
    if (status === "Requested") {
      inputData = inputData.filter((invoice) => invoice.record_status === 1);
    } else if (status === "approved") {
      const currentDate = new Date();
      inputData = inputData.filter((invoice) => (invoice.record_status === 2) && !(currentDate >= new Date(invoice.start_date) && currentDate <= new Date(invoice.till_date)) && !(currentDate > new Date(invoice.start_date)));
    } else if (status === "rejected") {
      inputData = inputData.filter((invoice) => invoice.record_status === 3);
    } else if (status === "active") {
      const currentDate = new Date();
      inputData = inputData.filter((invoice) => (invoice.record_status === 2) && currentDate >= new Date(invoice.start_date) && currentDate <= new Date(invoice.till_date));
    } else if (status === "expired") {
      const currentDate = new Date();
      inputData = inputData.filter((invoice) => (invoice.record_status != 1 && invoice.record_status != 3) && currentDate >= !(new Date(invoice.start_date) && currentDate <= new Date(invoice.till_date)) && currentDate > new Date(invoice.till_date));
    }
  }
