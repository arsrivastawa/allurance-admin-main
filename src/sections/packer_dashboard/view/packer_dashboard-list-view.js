...
import ProductTableFiltersResult from '../packer_dashboard-table-filters-result';
import {
  RenderCellShape,
...
} from '../packer_dashboard-table-row';
...
export default function ProductListView(permissions) {
  ...
  const getListingData = async () => {
    try {
      ...
      const response = await ManageAPIsDataWithHeader(PACKERS_DASHBOARD_RECORDS, fetchMethod);
  ...
  const getCardData = async () => {
    try {
      ...
      const response = await ManageAPIsDataWithHeader(PACKERS_DASHBOARD_CARD_DATA, fetchMethod);
  ...
  return (
    <>
      ...
      >
        <Typography variant="h3">Packers Dashboard</Typography>
