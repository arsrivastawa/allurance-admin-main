...
import { MOBILE_MASTHEAD_ENDPOINT, ine_category_ModuleID } from '../../../utils/apiEndPoints';
import ProductTableFiltersResult from '../mmasthead-table-filters-result';
import {
  // RenderCellStock,
  // RenderCellPrice,
  RenderCellPublish,
...
export const metadata = {
  title: 'Dashboard: Mobile Masthead List',
};
...
export default function ProductListView(permissions) {
  ...
  const getListingData = async () => {
    try {
      ...
      const response = await ManageAPIsData(MOBILE_MASTHEAD_ENDPOINT, 'GET');
  ...
  const handleDeleteRow = useCallback(
    async (id) => {
      try {
        ...
        const apiUrl = `${MOBILE_MASTHEAD_ENDPOINT}?id=${id}`;
  ...
  const handleDeleteRows = useCallback(async () => {
    ...
    try {
      ...
      const apiUrl = `${MOBILE_MASTHEAD_ENDPOINT}?ids=${deletedIds}`;
      const response = await ManageAPIsData(apiUrl, 'DELETE');
  ...
  const handleEditRow = useCallback(
    (id) => {
      router.push(paths.dashboard.mmasthead.edit(id));
    },
    [router]
  );
      ...
      >
        <CustomBreadcrumbs
          ...
          links={[
            ...
            {
              name: 'Mobile Masthead',
              href: paths.dashboard.mmasthead.root,
            },
            { name: 'List' },
          ]}
          action=
          {add_access === 1 && (
            <Button
              ...
              href={paths.dashboard.mmasthead.new}
              variant="contained"
