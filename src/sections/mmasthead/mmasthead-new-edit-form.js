...
import { ManageAPIsData, createImageOption } from '../../utils/commonFunction';
import { MOBILE_MASTHEAD_ENDPOINT } from '../../utils/apiEndPoints';
...
export default function ProductNewEditForm({ currentProduct }) {
  ...
  const mastheadBasePath = '/assets/images/masthead/mmasthead/';
  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      ...
      const apiUrl = currentProduct ? `${MOBILE_MASTHEAD_ENDPOINT}?id=${currentProduct.id}` : MOBILE_MASTHEAD_ENDPOINT;
      const fetchMethod = currentProduct ? "PUT" : "POST";
      const response = await ManageAPIsData(apiUrl, fetchMethod, mappedData);

      if (response.ok) {
        enqueueSnackbar(currentProduct ? 'Update success!' : 'Create success!');
        router.push(paths.dashboard.mmasthead.root);
      } else {
