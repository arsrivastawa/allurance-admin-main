...
import { PAGES_ENDPOINT } from '../../utils/apiEndPoints';
...
export default function ProductNewEditForm({ currentProduct }) {
  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      ...
      const apiUrl = currentProduct ? `${PAGES_ENDPOINT}?id=${currentProduct.id}` : PAGES_ENDPOINT;
      const fetchMethod = currentProduct ? "PUT" : "POST";
      const response = await ManageAPIsData(apiUrl, fetchMethod, mappedData);

      if (response.ok) {
        enqueueSnackbar(currentProduct ? 'Update success!' : 'Create success!');
        router.push(paths.dashboard.pagesdata.root);
      } else {
