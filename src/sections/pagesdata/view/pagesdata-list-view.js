...
import { PAGES_ENDPOINT, ine_category_ModuleID } from '../../../utils/apiEndPoints';
import ProductTableFiltersResult from '../pagesdata-table-filters-result';
import {
  // RenderCellStock,
  // RenderCellPrice,
  RenderCellPublish,
...
export const metadata = {
  title: 'Dashboard: Pages Data List',
};
...
export default function ProductListView(permissions) {
  ...
  const getListingData = async () => {
    try {
      ...
      const response = await ManageAPIsData(PAGES_ENDPOINT, 'GET');
  ...
  const handleDeleteRow = useCallback(
    async (id) => {
      try {
        ...
        const apiUrl = `${PAGES_ENDPOINT}?id=${id}`;
  ...
  const handleDeleteRows = useCallback(async () => {
    ...
    try {
      ...
      const apiUrl = `${PAGES_ENDPOINT}?ids=${deletedIds}`;
      const response = await ManageAPIsData(apiUrl, 'DELETE');
  ...
  const handleEditRow = useCallback(
    (id) => {
      router.push(paths.dashboard.pagesdata.edit(id));
    },
    [router]
  );
    ...
    <>
      ...
      >
        <CustomBreadcrumbs
          ...
          //     href={paths.dashboard.pagesdata.new}
          //     variant="contained"
