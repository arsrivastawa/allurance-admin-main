...
import ProductTableFiltersResult from '../shape-table-filters-result';
import {
  // RenderCellStock,
  // RenderCellPrice,
  // RenderCellPublish,
...
export default function ProductListView(permissions) {
  ...
  const getListingData = async () => {
    try {
      ...
      const response = await ManageAPIsDataWithHeader(SHAPE_ENDPOINT, fetchMethod, { headers });
  ...
  const handleDeleteRows = useCallback(async () => {
    ...
    try {
      ...
      const apiUrl = `${SHAPE_ENDPOINT}?ids=${deletedIds}`;
      const response = await ManageAPIsData(apiUrl, 'DELETE');
  ...
  const handleEditRow = useCallback(
    (id) => {
      router.push(paths.dashboard.shape.edit(id));
    },
    [router]
  );
        ...
        <CustomBreadcrumbs
          ...
          action={add_access === 1 && (
            <Button
              ...
              href={paths.dashboard.shape.new}
              variant="contained"
