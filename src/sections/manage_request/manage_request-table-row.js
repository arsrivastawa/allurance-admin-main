...
export default function InvoiceTableRow({
  ...
  const { sent, invoiceNumber, created_at, dueDate, request_status, invoiceTo, totalAmount, gift_card_type } = row;
    ...
    <>
      <TableRow hover selected={selected}>
        ...
        <TableCell sx={{ display: 'flex', alignItems: 'center' }}>
          ...
          <Avatar alt='' sx={{ mr: 2 }}>
            {row?.user_first_name?.charAt(0).toUpperCase()}
          </Avatar>

          <ListItemText
            primary={`${row.user_first_name} ${row.user_last_name}`}
            secondary={row.user_prefix_id}
            primaryTypographyProps={{ typography: 'body2', noWrap: true }}
            secondaryTypographyProps={{
              mt: 0.5,
              component: 'span',
