...
import { MARKETING_ENDPOINT, MY_MARKETING_ENDPOINT } from '../../../utils/apiEndPoints';
import ProductTableFiltersResult from '../campaign-table-filters-result';
import {
  // RenderCellStock,
  // RenderCellPrice,
  RenderCellPublish,
...
export default function ProductListView(permissions) {
  ...
  const getListingData = async () => {
    try {
      ...
      const response = await ManageAPIsData(MY_MARKETING_ENDPOINT, 'GET');
  ...
  const handleAddMarketingData = useCallback(
    () => {
      router.push(paths.dashboard.campaign.add);
    },
    [router]
  );
  ...
  const columns = [
    ...
    //       label="Edit"
    //       onClick={() => handleAddMarketingData(params.row.id)}
    //     />,
    //     <GridActionsCellItem
    //       showInMenu
          ...
          update_access === 1 &&
            actions.push(
              <GridActionsCellItem
                ...
                icon={<Iconify icon="solar:pen-bold" />}
                label="Edit"
                onClick={() => handleAddMarketingData()}
              />
            )
        }
