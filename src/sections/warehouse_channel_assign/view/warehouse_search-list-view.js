...
import InvoiceAnalytic from '../warehouse_search-analytic';
import InvoiceTableRow from '../warehouse_search1-table-row';
import InvoiceTableToolbar from '../warehouse_search-table-toolbar';
import InvoiceTableFiltersResult from '../warehouse_search-table-filters-result';
import { DESIGNER_ENDPOINT, WARE, CHANNEL_ASSIGN_ENDPOINT } from '../../../utils/apiEndPoints';
import { fetchDataFromApi, ManageAPIsDataWithHeader } from '../../../utils/commonFunction';
// ----------------------------------------------------------------------

const TABLE_HEAD = [
  { id: 'carton_title', label: 'Carton Title ' },
  { id: 'rack_title', label: 'Rack Title ' },
  { id: 'rack_code', label: 'Rack Code ' },
  { id: 'created_at', label: 'Created at' },
  { id: 'is_shipped', label: 'Channel assigned' }
];
...
export default function InvoiceListView(permissions) {
  ...
  //   try {
  //     const apiUrl = CHANNEL_ASSIGN_ENDPOINT;
  ...
  const getListingData = async () => {
    try {
      ...
      const response = await ManageAPIsDataWithHeader(CHANNEL_ASSIGN_ENDPOINT, fetchMethod, { headers });
  ...
  const handleDeleteRows = useCallback(async () => {
    ...
    const apiUrl = `${WAREHOUSE_ENDPOINT}?ids=${deletedIds}`;
    const responseData = await fetchDataFromApi(apiUrl, 'DELETE');
  ...
  const handleEditRow = useCallback(
    (id) => {
      router.push(paths.dashboard.channelassign.edit(id));
    },
    [router]
  );
  ...
  const handleAddRow = useCallback(
    (id) => {
      router.push(paths.dashboard.channelassign.new(id));
    },
    [router]
  );
  ...
  const handleViewRow = useCallback(
    (id) => {
      router.push(paths.dashboard.channelassign.details(id));
    },
    [router]
  );
      ...
      <Container maxWidth={settings.themeStretch ? false : 'lg'}>
        <CustomBreadcrumbs
          ...
          links={[
            ...
            {
              name: 'Warehouse',
              href: paths.dashboard.channelassign.root,
            },
            {
              name: 'Channel Assign',
            },
          ...
          action={add_access === 1 && (
            <Button
              ...
              href={paths.dashboard.channelassign.new}
              variant="contained"
