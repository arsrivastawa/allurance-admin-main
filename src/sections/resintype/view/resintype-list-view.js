...
import { RESINTYPE_ENDPOINT } from '../../../utils/apiEndPoints';
import ProductTableFiltersResult from '../resintype-table-filters-result';
import {
  // RenderCellStock,
  // RenderCellPrice,
  // RenderCellPublish,
...
export default function ProductListView(permissions) {
  ...
  const getListingData = async () => {
    try {
      ...
      const response = await ManageAPIsDataWithHeader(RESINTYPE_ENDPOINT, fetchMethod, { headers });
  ...
  const handleDeleteRow = useCallback(
    async (id) => {
      try {
        ...
        const apiUrl = `${RESINTYPE_ENDPOINT}?id=${id}`;
  ...
  const handleDeleteRows = useCallback(async () => {
    ...
    try {
      ...
      const apiUrl = `${RESINTYPE_ENDPOINT}?ids=${deletedIds}`;
      const response = await ManageAPIsData(apiUrl, 'DELETE');
  ...
  const handleEditRow = useCallback(
    (id) => {
      router.push(paths.dashboard.resintype.edit(id));
    },
    [router]
  );
        ...
        <CustomBreadcrumbs
          ...
          action={add_access === 1 && (
            <Button
              ...
              href={paths.dashboard.resintype.new}
              variant="contained"
