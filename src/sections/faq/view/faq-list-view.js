...
import { FAQS_ENDPOINT, ine_category_ModuleID } from '../../../utils/apiEndPoints';
import ProductTableFiltersResult from '../faq-table-filters-result';
import {
  // RenderCellStock,
  // RenderCellPrice,
  RenderCellPublish,
...
export default function ProductListView(permissions) {
  ...
  const getListingData = async () => {
    try {
      ...
      const response = await ManageAPIsData(FAQS_ENDPOINT, 'GET');
  ...
  const handleDeleteRow = useCallback(
    async (id) => {
      try {
        ...
        const apiUrl = `${FAQS_ENDPOINT}?id=${id}`;
  ...
  const handleDeleteRows = useCallback(async () => {
    ...
    try {
      ...
      const apiUrl = `${FAQS_ENDPOINT}?ids=${deletedIds}`;
      const response = await ManageAPIsData(apiUrl, 'DELETE');
