...
import { OFFLINE_SALES_ENDPOINT, ine_offline_sales_ModuleID } from '../../../utils/apiEndPoints';
import ProductTableFiltersResult from '../offline_sales-table-filters-result';
import {
  // RenderCellStock,
  // RenderCellPrice,
  RenderCellPublish,
...
export default function ProductListView(permissions) {
  ...
  const getListingData = async () => {
    try {
      ...
      const response = await ManageAPIsData(OFFLINE_SALES_ENDPOINT, 'GET');
  ...
  const handleViewRow = useCallback(
    (id) => {
      router.push(paths.dashboard.offlinesales.details(id));
    },
    [router]
  );
      ...
      getActions: (params) => {
        ...
        // }
        {
          actions.push(<GridActionsCellItem
            ...
            label="View"
            onClick={() => handleViewRow(params.row.model_number)}
          />)
        }
      ...
      >
        <CustomBreadcrumbs
          ...
          {add_access === 1 && (<></>
            ...
            //   href={paths.dashboard.offlinesales.new}
            //   variant="contained"
