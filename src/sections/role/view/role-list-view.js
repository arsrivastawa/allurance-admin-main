...
import { ROLE_ENDPOINT } from '../../../utils/apiEndPoints';
import { ManageAPIsData, ManageAPIsDataWithHeader } from '../../../utils/commonFunction';
import ProductTableFiltersResult from '../role-table-filters-result';
import {
  // RenderCellStock,
  // RenderCellPrice,
  // RenderCellPublish,
...
export default function ProductListView(permissions) {
  ...
  const getListingData = async () => {
    try {
      ...
      const response = await ManageAPIsDataWithHeader(ROLE_ENDPOINT, fetchMethod, { headers });
  ...
  const handleDeleteRow = useCallback(
    async (id) => {
      try {
        ...
        const apiUrl = `${ROLE_ENDPOINT}?id=${id}`;
  ...
  const handleDeleteRows = useCallback(async () => {
    ...
    try {
      ...
      const apiUrl = `${ROLE_ENDPOINT}?ids=${deletedIds}`;
      const response = await ManageAPIsData(apiUrl, 'DELETE');
  ...
  const columns = [
    ...
    },
    {
      ...
      minWidth: 360,
      hideable: false,
      renderCell: (params) => <RenderCellPrefix params={params} />,
    },
    {
      field: 'created_at',
    ...
    },
    {
      ...
      getActions: (params) => {
        const hiddenActionsForIds = [1];
        if (hiddenActionsForIds.includes(params.row.id)) {
          return [];
        }
        const actions = [];
        {
