...
export function RenderCellCreatedAt({ params }) {
  return (
    <ListItemText
      primary={fDate(params.row.invoice_date)}
      secondary={fTime(params.row.invoice_date)}
...
export function RenderCellShape({ params }) {
  return (
      ...
      <ListItemText
        ...
        primary={
          <Link
            ...
            sx={{ cursor: 'pointer' }}
          >
            {params.row.ios_serial_number_id} 
          </Link>
        }
...
RenderCellSequenceNumber.propTypes = {
  params: PropTypes.shape({
    row: PropTypes.object,
  }),
};

export function RenderCellSequenceNumber({ params }) {
  return (
    <Stack direction="row" alignItems="center" sx={{ py: 2, width: 1 }}>
