...
import { SOCIALLINK_ENDPOINT } from '../../utils/apiEndPoints';
...
export default function ProductNewEditForm({ currentProduct }) {
  ...
  const NewProductSchema = Yup.object().shape({
    ...
    social_link: Yup.string().required('Link is required'),
  });

  const defaultValues = useMemo(
    () => ({
      title: currentProduct?.title || '',
      social_link: currentProduct?.social_link || '',
    }),
    [currentProduct]
  );
  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      ...
      const apiUrl = currentProduct ? `${SOCIALLINK_ENDPOINT}?id=${currentProduct.id}` : SOCIALLINK_ENDPOINT;
      const fetchMethod = currentProduct ? "PUT" : "POST";
      const response = await ManageAPIsData(apiUrl, fetchMethod, mappedData);

      if (response.ok) {
        enqueueSnackbar(currentProduct ? 'Update success!' : 'Create success!');
        router.push(paths.dashboard.social.root);
      } else {
      ...
      <Grid xs={12} md={12}>
        <Card>
          ...
          <Stack spacing={3} sx={{ p: 3 }}>
            ...
            <RHFTextField id="social_link" name="social_link" label="Link" />
