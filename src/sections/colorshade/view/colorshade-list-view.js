...
import ProductTableFiltersResult from '../colorshade-table-filters-result';
import {
  // RenderCellStock,
  // RenderCellPrice,
  // RenderCellPublish,
...
export default function ProductListView(permissions) {
  ...
  const handleDeleteRow = useCallback(
    async (id) => {
      try {
        ...
        const apiUrl = `${COLORSHADE_ENDPOINT}?id=${id}`;
  ...
  const handleDeleteRows = useCallback(async () => {
    ...
    try {
      ...
      const apiUrl = `${COLORSHADE_ENDPOINT}?ids=${deletedIds}`;
      const response = await ManageAPIsData(apiUrl, 'DELETE');
  ...
  const handleEditRow = useCallback(
    (id) => {
      router.push(paths.dashboard.colorshade.edit(id));
    },
    [router]
  );
      ...
      >
        <CustomBreadcrumbs
          ...
          links={[
            ...
            {
              name: 'Color Shade',
              href: paths.dashboard.colorshade.root,
            },
            { name: 'List' },
          ]}
          action={add_access === 1 && (
            <Button
              ...
              href={paths.dashboard.colorshade.new}
              variant="contained"
