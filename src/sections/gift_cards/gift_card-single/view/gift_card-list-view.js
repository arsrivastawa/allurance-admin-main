...
export default function ProductListView() {
  ...
  const getListingData = async () => {
    try {
      ...
      const response = await ManageAPIsDataWithHeader(GIFT_CARD_ENDPOINT + "?type=3", fetchMethod, { headers });
  ...
  const handleEditRow = useCallback(
    (id) => {
      router.push(paths.dashboard.single_gift_card.edit(id));
    },
    [router]
  );
  ...
  const handleViewRow = useCallback(
    (id) => {
      router.push(paths.dashboard.single_gift_card.details(id));
    },
    [router]
  );
        ...
        <CustomBreadcrumbs
          ...
          action={
            <Button
              ...
              href={paths.dashboard.single_gift_card.new}
              variant="contained"
