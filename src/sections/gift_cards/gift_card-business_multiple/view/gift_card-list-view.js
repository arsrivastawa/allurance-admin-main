...
import ProductTableFiltersResult from '../gift_card-table-filters-result';
import {
  // RenderCellStock,
  // RenderCellPrice,
  RenderCellPublish,
...
export default function ProductListView() {
  ...
  //     const response = await ManageAPIsData(GIFT_CARD_ENDPOINT + "?type=1", 'GET');
  //     if (!response.ok) {
  ...
  const getListingData = async () => {
    try {
      ...
      const response = await ManageAPIsDataWithHeader(GIFT_CARD_ENDPOINT + "?type=1", fetchMethod, { headers });
  ...
  const handleDeleteRows = useCallback(async () => {
    ...
    try {
      ...
      const apiUrl = `${GIFT_CARD_ENDPOINT}?ids=${deletedIds}`;
      const response = await ManageAPIsDataWithHeader(apiUrl, 'DELETE', { headers });
  ...
  const handleEditRow = useCallback(
    (id) => {
      router.push(paths.dashboard.gift_card.edit(id));
    },
    [router]
  );
  ...
  const handleViewRow = useCallback(
    (id) => {
      router.push(paths.dashboard.gift_card.details(id));
    },
    [router]
  );
    ...
    // },
    {
      ...
      getActions: (params) => [
        <GridActionsCellItem
          ...
          label="View"
          title={params.row.record_status}
        ...
        <CustomBreadcrumbs
          ...
          action={
            <Button
              ...
              href={paths.dashboard.gift_card.new}
              variant="contained"
