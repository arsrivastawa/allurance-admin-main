...
import { CATEGORY_ENDPOINT, ine_category_ModuleID, WAREHOUE_BOXES_ENDPOINT, WAREHOUE_RACKS_ENDPOINT, WAREHOUSE_ADD_NAME } from '../../../utils/apiEndPoints';
import ProductTableFiltersResult from '../warehouse_boxes-table-filters-result';
import {
  // RenderCellStock,
  // RenderCellPrice,
  RenderCellPublish,
  ...
  RenderCellStock,
} from '../warehouse_boxes-table-row';
...
export default function ProductListView(permissions) {
  ...
  const getListingData = async () => {
    try {
      ...
      const response = await ManageAPIsData(WAREHOUE_BOXES_ENDPOINT, 'GET');
  ...
  const handleDeleteRows = useCallback(async () => {
    ...
    try {
      ...
      const apiUrl = `${WAREHOUE_BOXES_ENDPOINT}?ids=${deletedIds}`;
      const response = await ManageAPIsData(apiUrl, 'DELETE');
  ...
  const handleEditRow = useCallback(
    (id) => {
      router.push(paths.dashboard.warehouse_boxes.edit(id));
    },
    [router]
  );
        ...
        <CustomBreadcrumbs
          ...
          {add_access === 1 && (
            <Button
              ...
              href={paths.dashboard.warehouse_boxes.new}
              variant="contained"
