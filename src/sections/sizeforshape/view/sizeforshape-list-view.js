...
import { SIZEFORSHAPE_ENDPOINT } from '../../../utils/apiEndPoints';
import ProductTableFiltersResult from '../sizeforshape-table-filters-result';
import {
  // RenderCellStock,
  // RenderCellPrice,
  // RenderCellPublish,
...
export default function ProductListView(permissions) {
  ...
  const getListingData = async () => {
    try {
      ...
      const response = await ManageAPIsDataWithHeader(SIZEFORSHAPE_ENDPOINT, fetchMethod, { headers });
  ...
  const handleDeleteRow = useCallback(
    async (id) => {
      try {
        ...
        const apiUrl = `${SIZEFORSHAPE_ENDPOINT}?id=${id}`;
  ...
  const handleDeleteRows = useCallback(async () => {
    ...
    try {
      ...
      const apiUrl = `${SIZEFORSHAPE_ENDPOINT}?ids=${deletedIds}`;
      const response = await ManageAPIsData(apiUrl, 'DELETE');
  ...
  const handleEditRow = useCallback(
    (id) => {
      router.push(paths.dashboard.sizeforshape.edit(id));
    },
    [router]
  );
      ...
      >
        <CustomBreadcrumbs
          ...
          links={[
            ...
            {
              name: 'Size for Shape',
              href: paths.dashboard.sizeforshape.root,
            },
            { name: 'List' },
          ]}
          action={add_access === 1 && (
            <Button
              ...
              href={paths.dashboard.sizeforshape.new}
              variant="contained"
