...
export function RenderCellStatus({ params }) {
  // Set record_status based on params.row.status
  const record_status =
    params.row.status === 1 ? 'active' :
      params.row.status === 2 ? 'banned' : 'active';
...
export function RenderCellOrders({ params }) {
  return (
    <Stack direction="row" alignItems="center" sx={{ py: 2, width: 1 }}>
