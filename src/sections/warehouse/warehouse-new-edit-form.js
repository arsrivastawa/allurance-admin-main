...
export default function ProductNewEditForm({ currentProduct }) {
  ...
  const FetchCartons = async (id) => {
    try {
      const apiUrl = id ? `${PACKERS_CARTONS_LIST_IN_WAREHOUSE_ENDPOINT}/${id}` : PACKERS_CARTONS_LIST_IN_WAREHOUSE_ENDPOINT;
  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      ...
      const apiUrl = currentProduct ? `${WAREHOUSE_ENDPOINT}?id=${currentProduct.id}` : WAREHOUSE_ENDPOINT;
