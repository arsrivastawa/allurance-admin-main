...
import UserTableRow from '../rewards-table-row';
import UserTableToolbar from '../rewards-table-toolbar';
import UserTableFiltersResult from '../rewards-table-filters-result';
import { DeleteMultipleRewards, DeleteRewards } from 'src/api/rewards';
...
export default function UserListView() {
  ...
  const canReset =
    !!filters.title || filters.status !== 'all' || (!!filters.start_date && !!filters.end_date);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(endpoints.rewards.list);
  ...
  const handleDeleteRows = useCallback(async () => {
    try {
      ...
      await DeleteMultipleRewards(selectedIds);
  ...
  const handleViewRow = useCallback(
    (id) => {
      router.push(paths.dashboard.rewards.view(id));
    },
    [router]
  );
  const handleImportRow = useCallback(
    (id) => {
      router.push(paths.dashboard.rewards.import(id));
    },
    [router]
  );
      ...
      <Container maxWidth={settings.themeStretch ? false : 'lg'}>
        <CustomBreadcrumbs
          ...
          links={[
            ...
            { name: 'Rewards', href: paths.dashboard.rewards.list },
            { name: 'List' },
          ]}
          action={
            <Button
              component={RouterLink}
              href={paths.dashboard.rewards.new}
              variant="contained"
                  ...
                  {dataFiltered
                    ...
                    .map((row) => (
                      <UserTableRow
                        ...
                        onImportRow={() => handleImportRow(row.id)}
                      />
                    ))}
                  <TableEmptyRows
                    height={denseHeight}
...
function applyFilter({ inputData, comparator, filters, dateError }) {
  const { title, description, status, role, start_date, end_date } = filters;
