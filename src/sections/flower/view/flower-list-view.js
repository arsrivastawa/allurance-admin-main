...
import { FLOWER_ENDPOINT } from '../../../utils/apiEndPoints';
import { ManageAPIsData, ManageAPIsDataWithHeader } from '../../../utils/commonFunction';
import ProductTableFiltersResult from '../flower-table-filters-result';
import {
  // RenderCellStock,
  // RenderCellPrice,
  // RenderCellPublish,
...
export default function ProductListView(permissions) {
  ...
  const getListingData = async () => {
    try {
      ...
      const response = await ManageAPIsDataWithHeader(FLOWER_ENDPOINT, fetchMethod, { headers });
  ...
  const handleDeleteRow = useCallback(
    async (id) => {
      try {
        ...
        const apiUrl = `${FLOWER_ENDPOINT}?id=${id}`;
  ...
  const handleDeleteRows = useCallback(async () => {
    ...
    try {
      ...
      const apiUrl = `${FLOWER_ENDPOINT}?ids=${deletedIds}`;
      const response = await ManageAPIsData(apiUrl, 'DELETE');
  ...
  const handleEditRow = useCallback(
    (id) => {
      router.push(paths.dashboard.flower.edit(id));
    },
    [router]
  );
        ...
        <CustomBreadcrumbs
          ...
          action={add_access === 1 && (
            <Button
              ...
              href={paths.dashboard.flower.new}
              variant="contained"
