...
// src/app/api/resintype/route.js
import { query } from "../../../utils/database";
import { ine_resin_ModuleID, ine_resin_tablename } from "../../../utils/apiEndPoints";
...
// Table Name
const tableName = ine_resin_tablename;
...
export const POST = async (req) => {
    try {
        ...
        // Insertion
        const insertResult = await query(`INSERT INTO ${tableName} (name, description, code,created_by) VALUES (?, ?, ?,?)`, [
            requestData.name,
...
export const PUT = async (req) => {
    try {
        ...
        const { name, description, code, apihitid } = await req.json();

        await query(`UPDATE ${tableName} SET name = ?, description = ?, code = ?,updated_by=?, updated_at = NOW() WHERE id = ?`, [name, description, code, , apihitid, id]);
...
export const DELETE = async (req) => {
    try {
        ...
        await Promise.all(deletedIds.map(async (deletedId) => {
            ...
            activityLog(ine_resin_ModuleID, currentRecord, null, 3, 0); // Maintain Activity Log
        }));
