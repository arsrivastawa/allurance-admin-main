...
import { ine_campaign_tablename, ine_marekting_tablename, ine_products_tablename } from "../../../utils/apiEndPoints";
...
const tableName3 = ine_products_tablename;

export const GET = async (req) => {
    try {
        const campaignIdentifier = getQueryParamId(new URL(req.url));
        let campaignData = null;

        // Fetch campaign data based on campaignIdentifier if provided
        if (campaignIdentifier) {
            const results = await query(`SELECT * FROM ${tableName} WHERE campaign_url LIKE ?`, [`%${campaignIdentifier}`]);
            if (results.length > 0) {
