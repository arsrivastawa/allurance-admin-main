...
// src/app/api/shape/route.js
import { query } from "../../../utils/database";
import { ine_shape_ModuleID, ine_shape_tablename } from "../../../utils/apiEndPoints";
...
// Table Name
const tableName = ine_shape_tablename;
...
export const POST = async (req) => {
    try {
        ...
        if (!requestData.sequence_number || !requestData.shape) {
            return sendResponse({ error: 'Sequence Number and Shape fields are required', status: false }, 400);
        }

        // Insertion
        const insertResult = await query(`INSERT INTO ${tableName} (sequence_number, description, shape,created_by) VALUES (?, ?, ?, ?)`, [
            requestData.sequence_number,
...
export const PUT = async (req) => {
    try {
        ...
        const { sequence_number, description, shape, apihitid } = await req.json();

        await query(`UPDATE ${tableName} SET sequence_number = ?, description = ?, shape = ?, updated_by=?, updated_at = NOW() WHERE id = ?`, [sequence_number, description, shape, apihitid, id]);
...
export const DELETE = async (req) => {
    try {
        ...
        await Promise.all(deletedIds.map(async (deletedId) => {
            ...
            activityLog(ine_shape_ModuleID, currentRecord, null, 3, 0); // Maintain Activity Log
        }));
