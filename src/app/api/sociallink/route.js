...
// src/app/api/sociallink/route.js
import { query } from "../../../utils/database";
import { ine_social_link_ModuleID, ine_social_link_tablename } from "../../../utils/apiEndPoints";
...
// Table Name
const tableName = ine_social_link_tablename;
...
export const POST = async (req) => {
    try {
        ...
        if (!requestData.title || !requestData.social_link) {
            return sendResponse({ error: 'Title and Link fields are required', status: false }, 400);
        }

        // Insertion
        const insertResult = await query(`INSERT INTO ${tableName} (title, social_link) VALUES (?, ?)`, [
            requestData.title,
            requestData.social_link,
        ]);
...
export const PUT = async (req) => {
    try {
        ...
        const { title, social_link } = await req.json();

        await query(`UPDATE ${tableName} SET title = ?, social_link = ?, updated_at = NOW() WHERE id = ?`, [title, social_link, id]);
...
export const DELETE = async (req) => {
    try {
        ...
        await Promise.all(deletedIds.map(async (deletedId) => {
            ...
            activityLog(ine_social_link_ModuleID, currentRecord, null, 3, 0); // Maintain Activity Log
        }));
